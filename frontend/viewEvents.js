...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        const response = await fetch(buildApiUrl("user", "/users/events/"));
...
async function fetchNotificationCount(userId) {
    try {
        const response = await fetch(buildApiUrl("notification", `/notifications/${userId}/count`));
        if (!response.ok) {
...
async function getTicketPrice(eventId) {
    try {
        console.log(eventId);
        const response = await fetch(buildApiUrl("event", `/api/events/${eventId}/price`));
...
async function confirmBooking(eventId) {
    ...
    try {
        ...
        const response = await fetch(buildApiUrl("user", "/users/bookings/"), {
            method: "POST",
            headers: {

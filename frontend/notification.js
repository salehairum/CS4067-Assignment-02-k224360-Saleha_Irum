...
function removeNotification(element) {
    ...
    fetch(buildApiUrl("notification", `/notifications/delete/${bookingId}`), {
        method: "DELETE",
        headers: {
        ...
        .then(data => {
            if (data.message === "Notification deleted successfully") {
                ...
                return fetch(buildApiUrl("booking", `/bookings/${bookingId}/status`), {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json"
                    },
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        const response = await fetch(buildApiUrl("notification", `/notifications/${id}`));
